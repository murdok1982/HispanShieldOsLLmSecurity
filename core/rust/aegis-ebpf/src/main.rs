...
impl AegisTelemetry {
    pub async fn new() -> Result<Self, anyhow::Error> {
        ...
        let mut bpf = Bpf::load(include_bytes_aligned!(
            "/opt/hispanshield/core/rust/aegis-ebpf/target/bpfel-unknown-none/release/aegis-ebpf"
        ))?;

        let program: &mut TracePoint = bpf.program_mut("aegis_telemetry")
            .ok_or_else(|| anyhow::anyhow!("Failed to find eBPF program"))?
            .try_into()?;
        
        program.load()?;
        program.attach("sched", "sched_switch")?;
        
        info!(target: "aegis_ebpf", "eBPF telemetry loaded and attached to sched_switch");
        ...
        Ok(Self { bpf, telemetry })
    }
    
    pub async fn collect_metrics(&self) -> SystemTelemetry {
        // Read from eBPF maps or use direct kernel interfaces
...
async fn read_cpu_usage() -> f64 {
    // Read from /proc/stat (kernel ground truth, not psutil)
